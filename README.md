...
| [`agent-example-a2a`](agent-example-a2a/) | A2A protocol — `AgentCardService` (server identity + skills filtered by `active` + tags + examples), `/.well-known/agent.json` REST endpoint, `/a2a` JSON-RPC (`message/send` happy + `skillHint`→`forceSkill` + engine-failure → `status=failed` + `-32600/-32601/-32602` error paths), `HttpA2AClient` discover/sendTask/getTask round-trips via loop-back HTTP. 14 assertions, zero infra. |
| [`agent-example-skill-llm-overrides`](agent-example-skill-llm-overrides/) | Skill-level LLM overrides — `metadata.preferred-model` (alias routing), `metadata.temperature` and `metadata.max-tokens` (applied to every `ChatRequest`). 12 assertions with mocked `RecordingChatModel` beans per alias. Requires v1.2.17+. Zero infra. |
